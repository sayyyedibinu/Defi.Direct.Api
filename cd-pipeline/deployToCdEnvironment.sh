...
sed -i -e "s/{{BRANCH_NAME_LOWERCASE}}/$branchName/g" ./cd-pipeline/k8s/cd-deployment.yaml
sed -i -e "s/{{BUILD_NUMBER}}/$BUILD_NUMBER/g" ./cd-pipeline/k8s/cd-deployment.yaml
sed -i -e "s/{{HealthCheckOptions.Key}}/$CD_HEALTH_CHECK_KEY/g" ./cd-pipeline/k8s/cd-deployment.yaml

sed -i -e "s/{{BRANCH_NAME_LOWERCASE}}/$branchName/g" ./cd-pipeline/k8s/cd-deployment-mysql.yaml
sed -i -e "s/{{BUILD_NUMBER}}/$BUILD_NUMBER/g" ./cd-pipeline/k8s/cd-deployment-mysql.yaml
sed -i -e "s/{{HealthCheckOptions.Key}}/$CD_HEALTH_CHECK_KEY/g" ./cd-pipeline/k8s/cd-deployment-mysql.yaml
...
kubectl create -f ./cd-pipeline/k8s/cd-deployment.yaml
return_code=$?
...
function retryableWaitForDeployment(){
  ...
  echo "Rollout message: $ROLLOUT_MESSAGE"
  if [[ $ROLLOUT_STATUS != 0 ]]; then exit $ROLLOUT_STATUS; fi
}

echo Waiting for API deploy to finish...
retryableWaitForDeployment "defi-direct-api-api-$branchName-$BUILD_NUMBER"
...
class PendingComponent extends React.Component{
    constructor(props){
        ...
        if (_appData) {
            ...
            this.state.pendingPageFieldData = { fields: _appData.site.pendingPageFieldData };
    ...
    handleSliderChange = (value, index) => {
        ...
        let tempVar = this.state.pendingPageFieldData;
        tempVar.fields[index].currentValue = value;
        this.setState({ pendingPageFieldData: tempVar });
    };
    handleCheckboxchanged= (event, index) => {
        
        let tempVar = this.state.pendingPageFieldData;
        tempVar.fields[index].currentValue = event.target.checked;;
        this.setState({ pendingPageFieldData: tempVar ,chkboxVal:event.target.checked});
    };
    getFinalDecission(){
        ...
        for (let i = 0; i < rowcount; i++) {
            ...
            for (let j = 0; j < colcount; j++) {
                if (toggleValues[i + ',' + j] === false) {
                    children.push(<td key={j}>{cellValues[i + ',' + j]}</td>);
                } else {
                    if (cellFields[i + ',' + j] != 'undefined' && cellFields[i + ',' + j] != null && cellFields[i + ',' + j] != "") {
                        ...
                        /*children.push(<td>{this.state.Results[cellValues[i + ',' + j]]}</td>);*/
    ...
    getButton = (_obj) => {
        ...
        /*if(RuleHelper.getConditionalDecisionDisplay(_obj, this.state.Results) !== true) return "";*/
        try {
            if (_obj.action === "Routing") {
                ...
                return(<Button className={_obj.displayPositionDetails} variant="contained" style={{color: 'white', backgroundColor: this.state.AppData.settings.defaultPrimary}} onClick={()=>this.ResubmitToLos(this.state.Results.ApplicationNumber,this.state.Results.AppId)} >{_obj.title}</Button>);
            }
            else
    ...
    showApplication = ()=>
    {
        this.setState({ redirect : {
            ...
            state: {AppData:this.state.AppData,siteId:this.state.siteId,appId:this.state.Results.AppId,createNewApp:false,selectedPage:"Pending",Results:this.state.Results}
        }}); 
    }
    ResubmitToLos(Appnum,AppId)
    {
        ...
        let listArr = [];
        this.state.pendingPageFieldData.fields.forEach(arrObj => {
            if(arrObj.inputType === "Slider" || arrObj.inputType === "Checkbox"){
                listArr.push({id:arrObj.id,currentval:arrObj.currentValue});
            }        
        });
        let _self = this;
        let _postURL = config.API_BASE_URL+'Sites/resendToLos/'+ 'Pending/' + Appnum +'/'+ AppId + '/' +this.props.pageID;
    ...
    getEditButton(){
        if(Object.prototype.toString.call(this.props.location.state) === '[object Undefined]')
        {
                ...
                if(this.props.AppData.settings.editButtonDisplayLogic === "null") return false;
                if(this.props.AppData.settings.editButtonDisplayLogic === "null") 
                {
                   if (this.props.AppData.settings.editApplication) return true;
                }
                let _tempVar = {};
                _tempVar.displayLogic = true;
                    ...
                    <Grid container spacing={0} className="personalLoanWrapper outputPageWrapper">
                        ...
                        <Card className={this.state.policyCheck ? "policyCheckCnt checked" : "policyCheckCnt"}>
                            ...
                            <CardContent style={{"padding": "5px"}} className="resultCardCnt">
                                    {
                                        this.state.pendingPageFieldData.fields.map((_obj, index) => {
...
export default compose(withWidth())(PendingComponent);

...
sed -i -e "s/{{TEST_ENVIRONMENT_NAME}}/$TEST_ENVIRONMENT_NAME/g" ./defi-direct-ui/cd-pipiline/manual-deployments/k8s/test-deployment.yaml
sed -i -e "s/{{DROP_DATABASE}}/$DROP_DATABASE/g" ./defi-direct-ui/cd-pipiline/manual-deployments/k8s/test-deployment.yaml
...
kubectl apply -f ./defi-direct-ui/cd-pipiline/manual-deployments/k8s/test-deployment.yaml
return_code=$?
...
retryableWaitForDeployment "defi-directui-$TEST_ENVIRONMENT_NAME"
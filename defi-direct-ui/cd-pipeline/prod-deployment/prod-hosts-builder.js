...
try {
    ...
    let configs = yaml.loadAll(fs.readFileSync('./defi-direct-ui/cd-pipeline/prod-deployment/k8s/prod-deployment.ingress.yaml', 'utf8'));
    //console.log(yaml.dump_all(configs));
    //return;
    configs.forEach((config) => {
        try {
            if (config.kind === 'Ingress' && config.metadata.name === 'defi-directui-prod-ext-ingress-baseline') {
                const connString = 'Server=defi-apps-prod-db-direct-02.cluster-cktexlr9o6uj.us-east-1.rds.amazonaws.com;Database=direct;Uid=DirectProd;Pwd=Direct2231!!2;';
                ...
                connection.query('SELECT * from Clients', function (error, results, fields) {
                  ...
                  results.forEach((result) => {
                      ...
                      if (hostBase !== 'baseline') {
                        let newConfig = JSON.parse(JSON.stringify(config));
                        newConfig.metadata.name = 'defi-directui-prod-ext-ingress-' + hostBase;
...
`;
                  ...
                  fs.writeFileSync('./defi-direct-ui/cd-pipeline/prod-deployment/k8s/prod-deployment.ingress.yaml', output, 'utf8');
                  console.log(output);
                });
                connection.end();
            }

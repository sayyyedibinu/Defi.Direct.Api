...
kubectl apply -f ./defi-direct-ui/cd-pipeline/staging-deployment/k8s/staging-deployment.yaml
return_code=$?
...
echo Waiting for defi-directui-staging API deploy to finish...
retryableWaitForDeployment "defi-directui-staging" 